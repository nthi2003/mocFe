"use client"
import Header from "@/app/components/header";
import Sidebar from "@/app/components/sidebar";
import { Button } from "@/components/ui/button";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from "@/components/ui/dialog";
import { Edit, Plus, Trash2 } from "lucide-react";
import React, { useEffect, useState } from "react";
import { api } from '@/lib/axios';
import { Label } from "@/components/ui/label";
import { Input } from "@/components/ui/input";

interface Unit {
    id: string;
    name: string;
}

export default function UnitListPage() {
    const [units, setUnits] = useState<Unit[]>([]);
    const [loading, setLoading] = useState(false);
    const [formData, setFormData] = useState({
        name: ''
    })
    const [isDialogOpen, setIsDialogOpen] = useState(false);
    const [isEditMode, setIsEditMode] = useState(false);
    const [currenUnitId, setCurrentUnitId] = useState<string | null>(null);

    useEffect(() => {
        fetchUnits();
    }, []);

    const fetchUnits = () => {
        setLoading(true);
        api.get('/Unit')
            .then((res) => {
                if (res.data.success) {
                    setUnits(res.data.data);
                }
            })
            .catch(() => {
                // Handle error silently or show a toast notification
            })
            .finally(() => {
                setLoading(false);
            });
    }

    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const { id, value } = e.target;
        setFormData(prev => ({
            ...prev,
            [id]: value
        }));
    };

    const handleAddUnit = async () => {
        try {
            setLoading(true);
            const response = await api.post("/Unit/CreateUnit", {
                name: formData.name,
            });
            if (response.data.success) {
                fetchUnits();
                setIsDialogOpen(false);
                resetForm();
            }
        }
        catch {
            // Handle error silently or show a toast notification
        } finally {
            setLoading(false);
        }
    }

    const handleEditUnit = async () => {
        if (!currenUnitId) return;
        try {
            setLoading(true);
            const response = await api.put(`/Unit/UpdateUnit/${currenUnitId}`, {
                name: formData.name
            });
            if (response.data.success) {
                fetchUnits();
                setIsDialogOpen(false);
                resetForm();
            }
        }
        catch {
            // Handle error silently or show a toast notification
        } finally {
            setLoading(false);
        }
    }

    const handleDeleteUnit = async (id: string) => {
        if (confirm('Bạn có chắc chắn muốn xóa bàn này?')) {
            try {
                setLoading(true);
                const response = await api.delete(`/Unit/DeleteUnit/${id}`);
                if (response.data.success) {
                    fetchUnits();
                }
            } catch {
                // Handle error silently or show a toast notification
            } finally {
                setLoading(false);
            }
        }
    };

    const resetForm = () => {
        setFormData({
            name: ''
        });
        setIsEditMode(false);
        setCurrentUnitId(null);
    };

    const openEditDialog = (unit: Unit) => {
        setFormData({
            name: unit.name,
        });
        setIsEditMode(true);
        setCurrentUnitId(unit.id);
        setIsDialogOpen(true);
    };

    const handleSubmit = () => {
        if (isEditMode) {
            handleEditUnit();
        } else {
            handleAddUnit();
        }
    };

    return (
        <div>
            <Header />
            <div className="flex">
                <Sidebar />
                <div className="p-[30px] w-full">
                    <div>
                        <div className="flex justify-between items-center mb-4">
                            <h2 className="text-lg font-medium">Danh sách đơn vị</h2>
                            <Dialog open={isDialogOpen} onOpenChange={(open) => {
                                if (!open) resetForm();
                                setIsDialogOpen(open);
                            }}>
                                <DialogTrigger asChild>
                                    <Button className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white">
                                        <Plus className="h-4 w-4" />
                                        Thêm đơn vị mới
                                    </Button>
                                </DialogTrigger>
                                <DialogContent>
                                    <DialogHeader>
                                        <DialogTitle>{isEditMode ? 'Chỉnh sửa đơn vị' : 'Thêm đơn vị mới'}</DialogTitle>
                                    </DialogHeader>
                                    <div className="space-y-4 pt-4">
                                        <div>
                                            <Label htmlFor="name">Tên đơn vị</Label>
                                            <Input
                                                id="name"
                                                placeholder="Nhập tên đơn vị..."
                                                className="mt-1"
                                                value={formData.name}
                                                onChange={handleInputChange}
                                            />
                                        </div>
                                        <div className="flex justify-end gap-2">
                                            <Button
                                                variant="outline"
                                                onClick={() => setIsDialogOpen(false)}
                                            >
                                                Hủy
                                            </Button>
                                            <Button
                                                onClick={handleSubmit}
                                                disabled={loading}
                                            >
                                                {loading
                                                    ? isEditMode
                                                        ? 'Đang cập nhật...'
                                                        : 'Đang thêm...'
                                                    : isEditMode
                                                        ? 'Cập nhật'
                                                        : 'Thêm đơn vị'}
                                            </Button>
                                        </div>
                                    </div>
                                </DialogContent>
                            </Dialog>
                        </div>
                        <div className="border rounded-md overflow-hidden">
                            <table className="w-full">
                                <thead className="bg-gray-50 border-b">
                                    <tr>
                                        <th className="text-left py-3 px-4 font-medium text-gray-600 w-16">STT</th>
                                        <th className="text-left py-3 px-4 font-medium text-gray-600">Tên đơn vị</th>
                                        <th className="text-center py-3 px-4 font-medium text-gray-600 w-32">Xử lí</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {loading && units.length === 0 ? (
                                        <tr>
                                            <td colSpan={7} className="text-center py-4">
                                                Đang tải dữ liệu...
                                            </td>
                                        </tr>
                                    ) : units.length === 0 ? (
                                        <td colSpan={7} className="text-center py-4">
                                            Không có bàn nào
                                        </td>
                                    ) : (
                                        units.map((unit, index) => (
                                            <tr key={unit.id} className="border-b last:border-b-0 hover:bg-gray-50">
                                                <td className="py-3 px-4">{index + 1}</td>
                                                <td className="py-3 px-4">{unit.name}</td>
                                                <td className="py-3 px-4">
                                                    <div className="flex items-center justify-center gap-2">
                                                        <button
                                                            className="p-1 text-blue-600 hover:bg-blue-50 rounded"
                                                            onClick={() => openEditDialog(unit)}
                                                        >
                                                            <Edit className="h-4 w-4" />
                                                        </button>
                                                        <button
                                                            className="p-1 text-red-600 hover:bg-red-50 rounded"
                                                            onClick={() => handleDeleteUnit(unit.id)}
                                                        >
                                                            <Trash2 className="h-4 w-4" />
                                                        </button>
                                                    </div>
                                                </td>
                                            </tr>
                                        ))
                                    )}
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}